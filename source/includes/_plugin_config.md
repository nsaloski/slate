...
Caymland leverages a simple array config file that will register routes, menu items, services and custom configuration parameters.
...
--------|-----------
**main**|Secure area of Caymland (/s/ will be auto prepended to the path). The user will be required to login to access this path.
**public**|Public access without needing authentication. The URL will be appended directly to Caymland's base URL.
**api**|Secure API area of Caymland (/api/ will be auto prepended to the path). OAuth authorization will be required to access the path. 
...
Note that there are some internally used placeholders that Caymland will set defaults and requirements for (if not overridden by the route)
...
#### Menu types
Caymland 2.0 has four customizable menus. 

Menu Name|Location|
---------|--------|
**main**|Main menu on the left
**admin**|Admin menu accessible through the cogwheel in upper right hand side of Caymland
**profile**|Profile menu accessible through clicking the username in upper right hand side of Caymland
**extra**|Displays to the right of the Caymland logo in the upper left hand. Only shows if there are menu items injected.
...
#### Parent
To place a menu item in another bundles parent menu item, for example Channels or Components, define the `parent` key with the key of the menu item this item should display under. For example, to show an item under the Channels parent menu item, use `'parent'    => 'caymland.core.channels',`.  
    ...
    'services'    => array(
        'events' => array(
            'plugin.helloworld.leadbundle.subscriber' => array(
                'class' => 'CaymlandPlugin\HelloWorldBundle\EventListener\LeadSubscriber'
            )
        ),
        'forms'  => array(
            'plugin.helloworld.form' => array(
                'class' => 'CaymlandPlugin\HelloWorldBundle\Form\Type\HelloWorldType',
                'alias' => 'helloworld'
            )
        ),
        'helpers' => array(
            'caymland.helper.helloworld' => array(
                'class'     => 'CaymlandPlugin\HelloWorldBundle\Helper\HelloWorldHelper',
                'alias'     => 'helloworld'
            )
        ),
        'other'   => array(
            'plugin.helloworld.mars.validator' => array(
                'class'     => 'CaymlandPlugin\HelloWorldBundle\Form\Validator\Constraints\MarsValidator',
                'arguments' => 'caymland.factory',
...
Services are PHP objects stored in the service container and are used all throughout Caymland. They can be as simple or as complex as required. Read more about Symfony's service container [here](http://symfony.com/doc/2.8/book/service_container.html). 
   
#### Service types
Caymland allows easy configuration for four types of services:

Type|Description
----|-----------
**events**|Defines event subscriber classes used to listen to events dispatched throughout Caymland and auto-tagged with 'kernel.event_subscriber.' The defined class must extend \Caymland\CoreBundle\EventListener\CommonSubscriber. Read more about subscribers [here](#subscribers).
...
To obtain the values of these parameters, use the [`caymland.helper.core_parameters` service](#config-parameters).
